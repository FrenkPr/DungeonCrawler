...
int main(int argc, char **argv)
{
    ...
    // Init order packet
    int order_count_player_packets = 0;

    send_packet(s, player_id, first.x, first.y, order_count_player_packets, &sin, MESSAGE_JOIN);
    ...
    while (window_opened)
    {
        ...
        if (receive_len > 0)
        {
            ...
            if (receive_len == 12)
            {
                unsigned int *packet_ptr = (unsigned int *)server_packet;

                // packet_ptr[0] = message type
                // packet_ptr[1] = message counter
                // packet_ptr[2] = payload (player id)
                player_id = packet_ptr[2];
            ...
            else if (receive_len == 20)
            {
                ...
                printf("Players: %u %u %f %f\n", packet->message_type, packet->player_id, packet->x, packet->y);
                ...
                {
                    printf("Current player: %zu\n", i);

                    if (other_players[i].player_id == packet->player_id)
                    {
                        printf("Current player corresponding id: %zu\n", i);
        ...
        {
            // Send packet
            send_packet(s, player_id, first.x, first.y, order_count_player_packets, &sin, MESSAGE_POSITION);

            // printf("player id sent: %d\n", player_id);

            order_count_player_packets++;
