...
class Server:
    def server_init(self, address, port):
        ...
        # MESSAGE TYPE: 0 - Join \ 1 - Welcome \ 2 - Position \ 3 - Melee
        self.format = "<IIIff"
    ...
    def tick(self):
        ...
        if message_type == MESSAGE_JOIN:
            ...
            if sender in self.players:
                ...
                print("new player", sender, "ID:", player_id)
            ...
            player_id, player_order_packets, x, y = struct.unpack("<IIff", data[4:20])

            if sender not in self.players:
                print("Start the server first and then the clients")
                exit(-1)

            # print("player id dict: ", self.players[sender].player_id)
            # print("player id var: ", player_id)
            # print("player_order packets: ", player_order_packets)
            # print("player x: ", x)
            # print("player y: ", y)

            if self.players[sender].player_id != player_id:
                #If player id != sender -> Move another player
                print(f"invalid player_id for {sender}. ID: {player_id}")
            ...
            # print("x:", x, "y:", y, "from", sender, "player_id", player_id)
